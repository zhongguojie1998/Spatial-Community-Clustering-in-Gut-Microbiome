...
pathway.mat <- read.table(gzfile(paste0("./picrust2-2.3.0-b/", sample.name, ".out/pathways_out/path_abun_unstrat.tsv.gz")), header = TRUE)
dir.create("pathway.plots/")
row.names(pathway.mat) <- pathway.mat$pathway
pathway.mat$pathway = c()
...
for (i in 1:dim(pathway.mat)[2]) {
  ...
  dir.create(paste0("pathway.plots/", sample.name))
  ggsave(filename = paste0("pathway.plots/", sample.name, "/community.", as.character(i), ".top10.pathways.pdf"))
}
}